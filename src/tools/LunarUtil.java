 ...
 * Describe :
 * Author   : kd.fan
 * Time     : 2022/02/14  -  18:28
 */
public class LunarUtil {
    private static int monCyl, dayCyl, yearCyl;
    private static int year, month, day;
    private static boolean isLeap;
    private final int[] lunarInfo = {0x04bd8, 0x04ae0, 0x0a570, 0x054d5,
    ...
    private final String[] Gan = {"甲", "乙", "丙", "丁", "戊", "己", "庚", "辛",
            "壬", "癸"};
    private final String[] Zhi = {"子", "丑", "寅", "卯", "辰", "巳", "午", "未",
            "申", "酉", "戌", "亥"};
    private final String[] Animals = {"鼠", "牛", "虎", "兔", "龙", "蛇", "马", "羊",
            "猴", "鸡", "狗", "猪"};

    private final String[] nStr1 = {"日", "一", "二", "三", "四", "五", "六", "七",
            "八", "九", "十"};
    private final String[] nStr2 = {"初", "十", "廿", "卅", "　"};

    private final String[] monthName = {"正", "正", "二", "三", "四", "五", "六",
            "七", "八", "九", "十", "十一", "十二"};
    private final String[] yearName = {"零", "壹", "贰", "叁", "肆", "伍", "陆",
            "柒", "捌", "玖"};
    ...
    private void toTransferLunar(Date objDate) {
        int i, leap, temp = 0;
    ...
    public String getLunar(String year, String month, String day) {
        ...
        //农历日期
        toTransferLunar(sDObj);

        String s = "农历 " + "【" + Animals[sy] + "】" + cYear(getYear()) + "年" + " ";
        s += (getIsLeap() ? "闰" : "") + monthName[getMonth()] + "月" + (monthDays(getYear(), getMonth()) == 29 ? "小" : "大");
        s += cDay(getDay()) + " ";
    ...
    public String getDayLunarForTitle() {
        Calendar cl = Calendar.getInstance();
        Date sDObj = cl.getTime();
        //农历日期
        toTransferLunar(sDObj);
        return cyclical(getYearCyl()) + "年" + monthName[getMonth()] + "月" + cDay(getDay());
    }
}
